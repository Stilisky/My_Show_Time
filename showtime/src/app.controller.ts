...
export class AppController {
  ...
  async addEventToTag(
    @Param('tag_id') tag_id: string,
    @Param('event_id_id') event_id: string,
  ) {
  ...
  async updateEventSubP(
    @Param('id') id: string,
    @Body() upEvt: UpdateEventDto,
  ) {
  ...
  async saveUser(@Body() newUser: CreateUserDto, @Res() res: Response) {
    try {
      ...
      res
        .status(statusCode)
        .render('register.hbs', { errorMessage: error.message });
    }
  }

  @Get("/addfavoris/:id")
  @Redirect("/dashboard")
  async favoris(@Param("id") id: string, @Session() session) {
    const userid = session.userId
